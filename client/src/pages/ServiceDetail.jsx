...
export default function ServiceDetails() {
  ...
  useEffect(() => {
    if (ServiceStateData && ServiceStateData.length > 0) {
      ...
      let selected = ServiceStateData.find((x) => x._id == _id);
      setData(selected || null);

      // filter remaining services
      let others = ServiceStateData.filter((x) => x._id != _id);
            ...
            {relatedData.map((service) => (
              <div className="col-md-6 col-lg-4" key={service._id}>
                ...
                >
                  ...
                  <Link to={`/serviceDetail/${service._id}`} className="btn btn-secondary py-2">
                    View Details
                  </Link>
                </div>
              </div>
